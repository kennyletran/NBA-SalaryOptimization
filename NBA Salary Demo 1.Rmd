...
nbacopy <- subset(nba, select = -c(Season, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))

pgcopy <- subset(pg, select = -c(Season, Pos, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))

sgcopy <- subset(sg, select = -c(Season, Pos, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))

sfcopy <- subset(sf, select = -c(Season, Pos, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))

pfcopy <- subset(pf, select = -c(Season, Pos, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))

ccopy <- subset(c, select = -c(Season, Pos, Tm, G, GS, MP, FG, FGA, X2PM, X2PA, X2PP, TRB, eFG, PER, BPM, OBPM, DBPM, VORP, WS, WS48, ORBP, DRBP, TRBP, ASTP, STLP, BLKP, TOVP, Salary, SalaryIn, Cap, Experience))
...
#Here i split train, text x and y
xpgtrain <- model.matrix(pCap ~. , trainpg)[,-1]
xpgtest <- model.matrix(pCap ~. , testpg)[,-1]
...
```{r, echo = FALSE, message=FALSE}
xsgtrain <- model.matrix(pCap ~. , trainsg)[,-1]
xsgtest <- model.matrix(pCap ~. , testsg)[,-1]
...
```{r, echo = FALSE, message=FALSE}
xsftrain <- model.matrix(pCap ~. , trainsf)[,-1]
xsftest <- model.matrix(pCap ~. , testsf)[,-1]
...
```{r, echo = FALSE, message=FALSE}
xpftrain <- model.matrix(pCap ~. , trainpf)[,-1]
xpftest <- model.matrix(pCap ~. , testpf)[,-1]
...
```{r, echo = FALSE, message=FALSE}
xctrain <- model.matrix(pCap ~. , traincenter)[,-1]
xctest <- model.matrix(pCap ~. , testcenter)[,-1]
...
pgcopy2 <- pgcopy %>% select(TS, FGP, FTP, X3PM, X3PP, FTr, PF, FTA, DRB, AST, BLK, FTM, Age, TOV, PTS, X3PAr, pCap)

sgvar <- rfe(sgcopy[,1:22], sgcopy[,23], sizes=c(1:22), rfeControl=control)
predictors(sgvar)
plot(sgvar, type=c("g", "o"))

sgcopy2 <- sgcopy %>% select(TS, FTP, FGP, BLK, X3PP, X3PAr, TOV, FTr, PF, DRB, STL, X3PM, FTM, ORB, PTS, Age, X3PA, FTA, pCap)

sfvar <- rfe(sfcopy[,1:22], sfcopy[,23], sizes=c(1:22), rfeControl=control)
predictors(sfvar)
plot(sfvar, type=c("g", "o"))

sfcopy2 <- sfcopy %>% select(FGP, X3PAr, FTP, STL, FTM, PF, TOV, DRB, X3PM, TS, AST, FTA, X3PP, X3PA, Age, FTr, pCap)
...
ccopy2 <- ccopy %>% select(FGP, TS, X3PAr, FTr, FTA, FTM, STL, BLK, X3PP, X3PM, AST, FTP, Age, X3PA, DRB, PTS, pCap)
```
...
```{r, echo = FALSE, message=FALSE}
#Ridge for PG
xpgtrain2 <- model.matrix(pCap ~. , trainpg2)[,-1]
xpgtest2 <- model.matrix(pCap ~. , testpg2)[,-1]
...
```{r, echo = FALSE, message=FALSE}
#Ridge for SG
xsgtrain2 <- model.matrix(pCap ~. , trainsg2)[,-1]
xsgtest2 <- model.matrix(pCap ~. , testsg2)[,-1]
...
```{r, echo = FALSE, message=FALSE}
#Ridge for SF
xsftrain2 <- model.matrix(pCap ~. , trainsf2)[,-1]
xsftest2 <- model.matrix(pCap ~. , testsf2)[,-1]
...
```{r, echo = FALSE, message=FALSE}
#Ridge for Center
xctrain2 <- model.matrix(pCap ~. , traincenter2)[,-1]
xctest2 <- model.matrix(pCap ~. , traincenter2)[,-1]
...
elastic_regpg <- train(pCap ~ .,data = trainpg, method = "glmnet", preProcess = c("center", "scale"), tuneLength = 10, trControl = train_cont)
...
train_cont <- trainControl(method = "repeatedcv", number = 10, repeats = 5,search = "random", verboseIter = TRUE)
elastic_regsg <- train(pCap ~ .,data = trainsg, method = "glmnet", preProcess = c("center", "scale"), tuneLength = 10, trControl = train_cont)
...
train_cont <- trainControl(method = "repeatedcv", number = 10, repeats = 5,search = "random", verboseIter = TRUE)
elastic_regsf <- train(pCap ~ .,data = trainsf, method = "glmnet", preProcess = c("center", "scale"), tuneLength = 10, trControl = train_cont)
...
train_cont <- trainControl(method = "repeatedcv", number = 10, repeats = 5,search = "random", verboseIter = TRUE)
elastic_regpf <- train(pCap ~ .,data = trainpf, method = "glmnet", preProcess = c("center", "scale"), tuneLength = 10, trControl = train_cont)
...
train_cont <- trainControl(method = "repeatedcv", number = 10, repeats = 5,search = "random", verboseIter = TRUE)
elastic_regc <- train(pCap ~ .,data = traincenter, method = "glmnet", preProcess = c("center", "scale"), tuneLength = 10, trControl = train_cont)
...
finalResults1 <- cbind(trainpg, "a" = round(predictionspgtrain3,8))
names(finalResults1)[names(finalResults1) == 'a'] <- "Predicted pCap"
finalResults1$Diff <- round(finalResults1$Predicted - finalResults1$pCap, 8) 

finalResults2 <- cbind(testpg, "a" = round(predictionspgtest3,8))
names(finalResults2)[names(finalResults2) == 'a'] <- "Predicted pCap"
finalResults2$Diff <- round(finalResults2$Predicted - finalResults2$pCap, 8) 

finalResults3 <- cbind(trainsg, "a" = round(predictionssgtrain3,8))
names(finalResults3)[names(finalResults3) == 'a'] <- "Predicted pCap"
finalResults3$Diff <- round(finalResults3$Predicted - finalResults3$pCap, 8) 

finalResults4 <- cbind(testsg, "a" = round(predictionssgtest3,8))
names(finalResults4)[names(finalResults4) == 'a'] <- "Predicted pCap"
finalResults4$Diff <- round(finalResults4$Predicted - finalResults4$pCap, 8) 

finalResults5 <- cbind(trainsf, "a" = round(predictionssftrain3,8))
names(finalResults5)[names(finalResults5) == 'a'] <- "Predicted pCap"
finalResults5$Diff <- round(finalResults5$Predicted - finalResults5$pCap, 8) 

finalResults6 <- cbind(testsf, "a" = round(predictionssftest3,8))
names(finalResults6)[names(finalResults6) == 'a'] <- "Predicted pCap"
finalResults6$Diff <- round(finalResults6$Predicted - finalResults6$pCap, 8) 

finalResults7 <- cbind(trainpf, "a" = round(predictionspftrain3,8))
names(finalResults7)[names(finalResults7) == 'a'] <- "Predicted pCap"
finalResults7$Diff <- round(finalResults7$Predicted - finalResults7$pCap, 8) 

finalResults8 <- cbind(testpf, "a" = round(predictionspftest3,8))
names(finalResults8)[names(finalResults8) == 'a'] <- "Predicted pCap"
finalResults8$Diff <- round(finalResults8$Predicted - finalResults8$pCap, 8) 

finalResults9 <- cbind(traincenter, "a" = round(predictionsctrain3,8))
names(finalResults9)[names(finalResults9) == 'a'] <- "Predicted pCap"
finalResults9$Diff <- round(finalResults9$Predicted - finalResults9$pCap, 8) 

finalResults10 <- cbind(testcenter, "a" = round(predictionsctest3,8))
names(finalResults10)[names(finalResults10) == 'a'] <- "Predicted pCap"
